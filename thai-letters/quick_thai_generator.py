...
def generate_output_name(samples):
    ...
    return f"thai_optimized_{category}_{samples}samples_{timestamp}"

def run_generator(samples, output_dir):
    """เรียกใช้ optimized generator"""
    command = [
        "python", 
        "optimized_thai_generator.py", 
        str(samples),
        "-d", "th_dict.txt",
        "-o", output_dir
    ]
    ...
    try:
        ...
        print(f"📋 Statistics: {output_dir}/optimized_dataset_details.json")
        print("=" * 60)
        ...
        print("\n❌ Error: optimized_thai_generator.py not found")
